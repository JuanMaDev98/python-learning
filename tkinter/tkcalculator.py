...
root = Tk()
root.title("TK-calculator")# cambia el titulo de la ventana
list_sum = []
# Funciones de los botones

def button_click(num):
    #entry1.delete(0, END) #Borra todo lo escrito en el entry1
    entry1.insert(END, num)# end inserta el numero en la posicion final
...
def button_substract_sign(lista):
    lista.append(int(entry1.get()))
    entry1.delete(0, END)
    global sign
    sign = "-"
...
button1 = Button(root, text = "1", padx = 30, pady = 15, command = lambda: button_click(1)) # Lambda antes de la funcion permite que se le pasen parametros a la funcion
...
button_substract = Button(root, text = "-", padx = 30, pady = 15, command = lambda: button_substract_sign(list_sum))
...
entry1.grid(row = 0, column = 0, columnspan = 3, padx = 10, pady =10) #Columnspan cambia la cantidad de columnas que usa el objeto para ocupar como espacio
...
button_substract.grid(row =6, column =0, columnspan = 1)
